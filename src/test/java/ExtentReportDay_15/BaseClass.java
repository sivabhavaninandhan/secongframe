...
public class BaseClass {
  ...
  {
	  ExtentSparkReporter sparkreporter=new ExtentSparkReporter("AllTestss.html");
  ...
  {
	  extentReports.flush();
	  Desktop.getDesktop().browse(new File("AllTestss.html").toURI());
  }
  @AfterMethod
  public void checkstatus(Method m,ITestResult result) throws IOException
  {
	  if(result.getStatus()==result.FAILURE)
	  {
		  ...
		  screenshotpath=Capturscreen(result.getTestContext().getName()+"_"+result.getMethod().getMethodName()+".png");
  ...
  public String Capturscreen(String imagename) throws IOException
  {
	  Date dt=new Date();
	  String timestamp=dt.toString().replace(":", "_").replace(" ", "");
	  
	  String path="./screen010/"+timestamp+imagename;
